...
async function updatePage(pageId, { title, sections }) {
  ...
  //    Phân trang để xóa hết cả khi page > 100 block (tối đa 5 trang cho an toàn).
...
// - pageId rỗng  -> tạo mới.
// - pageId có    -> cập nhật; CHỈ khi page không còn (404, hoặc đã archive) mới tạo mới.
//   Lỗi 400 khác (validate, sai property...) phải nổi lên — tránh tạo page trùng lặp.
export async function upsertNotionExportPage({ pageId, title, sections }) {
  ...
  if (pageId) {
    try {
      ...
      const archived = e.status === 400 && /archived/i.test(String(e.message || ""));
      if (e.status === 404 || archived) {
