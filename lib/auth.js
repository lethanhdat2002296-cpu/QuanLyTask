...
export async function getCurrentUser() {
  ...
  const rows = await sql`
    SELECT token_version, is_admin FROM users WHERE id = ${Number(payload.sub)}
  `;
  ...
  payload.isAdmin = rows[0].is_admin === true;
  return payload;
}
...
// Lấy ngữ cảnh đăng nhập đầy đủ: { id, username, isAdmin } hoặc null
export async function getAuth() {
  const user = await getCurrentUser();
  if (!user) return null;
  const id = Number(user.sub);
  if (!Number.isFinite(id)) return null;
  return { id, username: user.username, isAdmin: user.isAdmin === true };
}
